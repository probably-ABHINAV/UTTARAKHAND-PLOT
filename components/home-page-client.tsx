...
export default function HomeClient({ initialPlots }: HomeClientProps) {
  const { toast } = useToast();

  // Normalize DB plots to UI pots
  const plotData = initialPlots.map(p => ({
    id: p.id,
    title: p.title,
    slug: p.slug,
    location: p.location,
    locationLink: "",
    size: `${p.size_sqyd} sq yd`,
    type: "Residential Plot",
    price: `₹${parseFloat(p.price).toLocaleString('en-IN')}`,
    features: p.description ? [p.description.substring(0, 30) + (p.description.length > 30 ? "..." : "")] : ["Prime Location"],
    images: p.images && p.images.length > 0 ? p.images : ["/placeholder.jpg"], // Fallback image needed
    rating: 4.8,
    reviews: 0,
    available: 1,
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      {/* --- PLOTS SECTION --- */}
      <section id="plots" className="py-20 bg-slate-50">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredPlots.length === 0 ? (
              <div className="col-span-full text-center py-10">
                <p className="text-muted-foreground text-lg">No plots available at the moment. Please check back later!</p>
              </div>
              ...
              >
                ...
                <div className="relative h-64 overflow-hidden bg-slate-200">
                  ...
                  <div className="absolute top-4 right-4 flex gap-2">
                    ...
                    >
                      <Heart
                        className={`h-4 w-4 ${favorites.includes(plot.id)
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {[
              ...
              <div key={idx} className="relative group overflow-hidden rounded-xl cursor-pointer shadow-sm aspect-[4/3] bg-slate-100">
                <Image
                  src={src}
