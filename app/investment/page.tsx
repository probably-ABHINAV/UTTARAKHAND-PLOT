...
export default function InvestmentPage() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-[#F7931E] 50">
      <SiteHeader />

      {/* Hero Section */}
      <section className="relative py-24 bg-gradient-to-br from-purple-600/20 via-blue-400/10 to-[#FF6B35]/60/20">
        <div className="container mx-auto px-4 text-center">
          <h1 className="font-bold text-5xl md:text-6xl mb-6 bg-gradient-to-r from-[#F7931E] to-[#FF6B35] bg-clip-text text-transparent">
            ...
            {investmentReasons.map((reason, index) => (
              <Card key={index} className="p-8 hover:shadow-xl transition-all duration-300 border-0 shadow-lg">
                <div className="flex items-start gap-4">
                  <div className="w-16 h-16 bg-gradient-to-r from-[#F7931E] to-[#FF6B35] rounded-full flex items-center justify-center flex-shrink-0">
                    ...
                    <ul className="space-y-2">
                      {reason.details.map((detail, idx) => (
                        <li key={idx} className="flex items-start gap-2 text-sm">
                          <div className="w-2 h-2 bg-[#F7931E] rounded-full mt-2 flex-shrink-0"></div>
                          {detail}
                        </li>
                      ))}
                    </ul>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <Card className="p-8 border-0 shadow-lg">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-2xl">
                  <BarChart3 className="h-6 w-6 text-[#F7931E]" />
                  ...
                  <div key={index} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3 flex-1 ml-6">
                      <div className="flex-1 bg-gray-200 rounded-full h-4 relative overflow-hidden">
                        <div 
                          className="absolute top-0 left-0 h-full bg-gradient-to-r from-[#F7931E] 400 to-[#F7931E]/50 rounded-full transition-all duration-1000"
                          style={{ width: `${(data.growth / 50) * 100}%` }}
                        ></div>
                      </div>
                      <span className="font-bold text-xl text-[#F7931E]">{data.growth}%</span>
                    </div>
                  </div>
                ))}
              </CardContent>
            ...
            <Card className="p-8 border-0 shadow-lg">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-2xl">
                  <PieChart className="h-6 w-6 text-[#FF6B35]" />
              ...
              <CardContent>
                <div className="space-y-4">
                  {marketAnalysis.comparisonData.map((data, index) => (
                    <div key={index} className={`p-4 rounded-lg ${index === 0 ? 'bg-gradient-to-r from-[#FF6B35] 100 to-[#F7931E] 100 border-2 border-purple-300' : 'bg-gray-50'}`}>
                      <div className="flex justify-between items-center mb-2">
                        <span className="font-bold text-lg">{data.location}</span>
                        <Badge className={index === 0 ? "bg-[#F7931E]" : "bg-gray-500"}>
                          {data.growth}% Growth
                        </Badge>
                      </div>
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <div className="font-semibold text-[#F7931E]">{data.returns}</div>
                        </div>
                      </div>
                    </div>
                  ))}
      ...
      <section className="py-16 bg-gradient-to-r from-[#F7931E] to-[#FF6B35] text-white">
        <div className="container mx-auto px-4 text-center">
            ...
            {[
              ...
              <Card key={index} className="p-6 hover:shadow-xl transition-all duration-300 border-0 shadow-lg">
                <CardHeader className="text-center">
                  <div className="w-16 h-16 bg-gradient-to-r from-[#F7931E] 400 to-[#F7931E]/50 rounded-full flex items-center justify-center mx-auto mb-4">
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-center">
                    <div className="p-3 bg-blue-50 rounded">
                      <div className="font-bold text-[#FF6B35]">{story.investment}</div>
                      <div className="text-xs text-gray-600">Invested</div>
                    </div>
                    <div className="p-3 bg-green-50 rounded">
                      <div className="font-bold text-[#F7931E]">{story.returns}</div>
                      <div className="text-xs text-gray-600">Current Value</div>
                    </div>
                  </div>
                  <div className="text-center">
                    <Badge className="bg-[#F7931E] mb-3">{story.timeline}</Badge>
